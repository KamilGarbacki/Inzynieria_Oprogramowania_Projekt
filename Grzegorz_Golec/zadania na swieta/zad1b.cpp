...
class GOLECPizzaBuilder : public PizzaBuilder {
public:
    void buildDough() override
    {
        pizza.setDough("Ultra THICC");
    }
    void buildSauce() override
    {
        pizza.setSauce("Green tomato sauce");
    }
    void buildTopping() override
    {
        pizza.setTopping("Pepe-roni meme pattern");
    }
    Pizza getPizza() const override { return pizza; }
private:
    Pizza pizza;
...
int main()
{
    ...
    GOLECPizzaBuilder grzegorz;
    cook.makePizza(grzegorz);
    Pizza golecpizza = grzegorz.getPizza();
    std::cout<<std::endl<<"My pizza:"<<std::endl;
    golecpizza.displayPizza();
